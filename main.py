...
class MainPage(PublicPage):
  def get(self,page):
    ...
    template_value={"images":images[:30],"prev":prev,"next":next, "logout":users.create_logout_url("/")}
...
class FindPage(PublicPage):
        def get(self):
            template_value = {}
            self.render('views/find.html', template_value)
